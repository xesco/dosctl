...
def list_games(collection, sort_by, installed):
    ...
    if installed:
        installed_base = collection.installed_dir_for(INSTALLED_DIR)
        installed_ids = {p.name for p in installed_base.iterdir() if p.is_dir()} if installed_base.exists() else set()
